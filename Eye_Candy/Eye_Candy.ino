...
void setup() {
  ...
  for (int i = 0; i < NUMS; i++) v[(1+rand()%(WIDTH-2))+(1+rand()%(HEIGHT-2))*WIDTH] = randomf(0.0f, TWO_PI);
...
void loop() {
  ...
  for (int j=0; j<HEIGHT; j++) {
    ...
    for (int i=0; i<WIDTH; i++) {
      ...
      uint8_t coll = 255.0f * sinf(p[i+j*WIDTH]);    
      if(coll < 192) display.drawPixel(i, j, BLACK);
